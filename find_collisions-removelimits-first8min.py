...
def find_collisions_per_video(datfile, csvs_path):
	...
	for sub in range(1,num_tads): # calculate for all
		if sub == def_num: #skip deformed lol
			continue 
		result = find_collisions(sub, def_num, csvs_path, wrong_distances, def_start_time, coll_times)
		sub_collisions = result[0]
		coll_times = result[2]

		all_collisions.append(result[0])
		total_collisions += len(result[0])
		time_with_def = result[1]
...
def find_collisions(wt_num, def_num, csvs_path, wrong_distances, def_start, coll_times):
	global ones
	deformed_csv = os.path.join(csvs_path, f"track-arena_1-subject_{def_num}.csv")
	...
	return (collisions, time_with_def, coll_times)
...
for i in range(len(filtered_collision_masterlist)):
	#print(i)
	if filtered_flat_velocities[i] > 150: # temporary upper limit is 200
